...
class InvertedIndex:
    ...
    postings_dict: Dictionary mapping:
        ...
        yang memetakan term ID (integer) ke 4-tuple:
           1. start_position_in_index_file : (dalam satuan bytes) posisi dimana
              postings yang bersesuaian berada di file (storage). Kita bisaf
    ...
    def __enter__(self):
        ...
        with open(self.metadata_file_path, 'rb') as f:
            self.postings_dict, self.terms, self.doc_length, self.nilai_N = pickle.load(f)
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        with open(self.metadata_file_path, 'wb') as f:
            pickle.dump([self.postings_dict, self.terms, self.doc_length, self.nilai_N], f)
...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        pos, _, len_in_bytes_of_postings, len_in_bytes_of_tf = self.postings_dict[term]
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        # Encode postings list dan tf list
        encoded_postings = self.postings_encoding.encode(postings_list)
        encoded_tf = self.postings_encoding.encode_tf(tf_list)
        
        # Simpan posisi saat ini di file
        curr_position = self.index_file.tell()
        
        # Tulis encoded postings dan tf ke file
        self.index_file.write(encoded_postings)
        self.index_file.write(encoded_tf)
        
        # Update postings dictionary dengan metadata
        self.postings_dict[term] = (curr_position, len(postings_list), len(encoded_postings), len(encoded_tf))
    ...
    def save_doc_lengths(self):
        self.nilai_N = len(self.doc_length)
