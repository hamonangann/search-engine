...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self):
        ...
        # TODO
        try:
            next_term_id = next(self.term_iter)
        except StopIteration:
            return (None, [])
        term, postings_list = self.get_postings_list(next_term_id)
    ...
    def get_postings_list(self, term):
        ...
        if self.postings_dict.get(term) == None:
            return (None, [])
        start_position_in_index_file, number_of_postings_in_list, length_in_bytes_of_postings_list = self.postings_dict.get(term)
        self.index_file.seek(start_position_in_index_file)
        # decode postings list dari start pos hingga len
        bytes_to_decode = self.index_file.read(length_in_bytes_of_postings_list)
        res = self.encoding_method.decode(bytes_to_decode)
        return (term, res)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list):
        ...
        """
        # TODO
        cur_pos=self.index_file.tell()
        encoded_postings_list = self.encoding_method.encode(postings_list)

        self.index_file.write(encoded_postings_list)

        las_pos=self.index_file.tell()
        
        self.postings_dict[term] = (cur_pos, len(postings_list), las_pos-cur_pos)
...
if __name__ == "__main__":
    ...
    with InvertedIndexWriter('test', encoding_method=VBEPostings, path='./tmp/') as index:
        index.append(1, [2, 3, 4, 8, 10])
        index.append(2, [3, 4, 5])
        index.index_file.seek(0)
        assert index.terms == [1,2], "terms salah"
